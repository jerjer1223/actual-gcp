  ...
  write_files = [
    ...
    },
    {
      ...
      content     = <<-EOT3
                ...
                [Service]
                ExecStart=/usr/bin/docker run --rm --network custom-bridge -p '[::1]:5006:5006' --mount 'type=bind,source=/mnt/disks/data/actual-data,target=/data'
                --mount 'type=bind,source=/mnt/disks/data/akahu_to_budget/.env,target=/.env' --name=actual_server gcr.io/budgetactual-447300/actual-custom:latest
